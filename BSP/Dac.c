...
static adc_cali_handle_t s_adc1_io7_cali_handle;
static bool s_adc_cali_enabled;
static bool s_adc_io4_cali_enabled;
static bool s_adc_io7_cali_enabled;
...
static const adc_channel_t ADC_IO7_CHANNEL = ADC_CHANNEL_6;
static const adc_atten_t ADC_IO7_ATTEN = ADC_ATTEN_DB_12;
...
esp_err_t adc_io6_init(void)
{
    ...
    if (err != ESP_OK)
    {
        ESP_LOGE(TAG, "adc_oneshot_config_channel(IO6) failed: %s", esp_err_to_name(err));
        return err;
    }
    ...
    adc_oneshot_chan_cfg_t io7_chan_cfg = {
        .atten = ADC_IO7_ATTEN,
        .bitwidth = ADC_BITWIDTH_DEFAULT,
    };
    err = adc_oneshot_config_channel(s_adc1_handle, ADC_IO7_CHANNEL, &io7_chan_cfg);
    if (err != ESP_OK)
    {
        ESP_LOGE(TAG, "adc_oneshot_config_channel(IO7) failed: %s", esp_err_to_name(err));
        return err;
    }
    ...
    else
    {
        ...
        ESP_LOGW(TAG, "adc calibration unavailable on GPIO%d, use approximate conversion", ADC_IO6_GPIO);
    }
    ...
    err = adc_calibration_init(ADC_UNIT_1, ADC_IO7_CHANNEL, ADC_IO7_ATTEN, &s_adc1_io7_cali_handle);
    if (err == ESP_OK)
    {
        s_adc_io7_cali_enabled = true;
        ESP_LOGI(TAG, "adc calibration enabled on GPIO%d", ADC_IO7_GPIO);
    }
    else
    {
        s_adc_io7_cali_enabled = false;
        ESP_LOGW(TAG, "adc calibration unavailable on GPIO%d, use approximate conversion", ADC_IO7_GPIO);
    }

    s_adc_inited = true;
    ESP_LOGI(TAG, "adc_io6_init done, IO4/IO6/IO7 channels configured");
    return ESP_OK;
}
...
esp_err_t adc_io7_get_voltage_mv(int *voltage_mv, int *raw)
{
    if (!voltage_mv)
    {
        return ESP_ERR_INVALID_ARG;
    ...
    int raw_code = 0;
    esp_err_t err = adc_oneshot_read(s_adc1_handle, ADC_IO7_CHANNEL, &raw_code);
    if (err != ESP_OK)
    {
        return err;
    }
    ...
    int mv = 0;
    if (s_adc_io7_cali_enabled)
    {
        err = adc_cali_raw_to_voltage(s_adc1_io7_cali_handle, raw_code, &mv);
        if (err != ESP_OK)
        {
            return err;
        }

...
void RunDataPeripheral(void) {
	...
	for (;;) {
		...
		if (opcode == OPCODE_TRIGGER) {
			...
			ESP_LOGI(TAG, "Trigger received. sig=%d cath=%d mon=%d", raw_signal,
					 raw_cath, raw_mon);
		...
		if (opcode == OPCODE_TMP_ALERT || opcode == STATUS) {
			ESP_LOGI(TAG,
					 "Temp alert or status request. Sending timeline snapshot");
			timeline_send_now();
			continue;
		}
		...
		if (opcode == NACK) {
			uint32_t id = ParsePkgIDFromCmd(&cmd);
			ESP_LOGI(TAG,
					 "NACK received for pkgID=%u (real data cannot retransmit)",
					 (unsigned)id);
			continue;
		}

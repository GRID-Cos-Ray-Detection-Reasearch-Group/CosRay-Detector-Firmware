        ...
        if (xQueueReceive(uart_queue, &event, portMAX_DELAY)) {
            if (event.type == UART_DATA) {
                ...
                if (len > 0) {
					...
					if (payload > CMD_LENGTH-1) payload = CMD_LENGTH-1;
...
static void BlueToothTxTask(void *pvParameters) {
    ...
    while (1) {
        ...
        if (xQueueReceive(TxQueue, &TxPkg, portMAX_DELAY)) {
            ESP_LOGI(TAG, "Sending BLE data (%u bytes)", (unsigned)TxPkg.length);
...
void AppSetup(void) {
    ...
    if (!CommandQueue) { ESP_LOGE(TAG, "Failed to create CommandQueue"); return; }

    DataQueue = xQueueCreate(DATA_QUEUE_SIZE, sizeof(Command_t));
    if (!DataQueue) { ESP_LOGE(TAG, "Failed to create DataQueue"); return; }

    TxQueue = xQueueCreate(TX_QUEUE_SIZE, sizeof(TxPkg_t));
    if (!TxQueue) { ESP_LOGE(TAG, "Failed to create TxQueue"); return; }

    FlashQueue = xQueueCreate(FLASH_QUEUE_SIZE, sizeof(TxPkg_t));
    if (!FlashQueue) { 
        ESP_LOGE(TAG, "Failed to create FlashQueue"); return; }
    ...
    // 创建任务
    xTaskCreate(AppBlueTooth, "BlueToothTask",
                BLUETOOTH_TASK_STACK_SIZE, NULL,
    ...
    xTaskCreate(BlueToothTxTask, "BlueToothTxTask",
                BLUETOOTH_TX_TASK_STACK_SIZE, NULL,
                BLUETOOTH_TX_TASK_PRIORITY, &bluetoothTxTaskHandle);
    ESP_LOGI(TAG, "BlueToothTxTask created successfully");

    xTaskCreate(AppDataStore, "DataStoreTask",
                DATA_STORE_TASK_STACK_SIZE, NULL,
